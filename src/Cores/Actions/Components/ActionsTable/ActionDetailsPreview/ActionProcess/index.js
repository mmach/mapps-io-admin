...
export function ActionProcess(props) {
    ...
    React.useEffect(() => {
        ...
        processesSelector.forEach(process => {
            process.process_chain.forEach(process_chain => {
                process_chain.process_chain_actions.forEach(process_chain_action => {
                    if (process_chain_action.action_id == props.actionsProject.id) {
